...
namespace NeutrinoFluxReweight{ 
  ...
  ThinTargetMC::ThinTargetMC(){
    ...
    for(int i=0;i<spart_prod.size();i++){
      fpC_pi[i] = new TFile(Form("%s/MC/FTFP/invxs_%s_FTFP_BERT.root",dirData,spart_prod[i].c_str()),"read");
      for(int j=0;j<mom_inc.size();j++)vpC_pi[i].push_back((TH2D*)fpC_pi[i]->Get(Form("xF_pT_%dGeV",mom_inc[j])));
    }
  }
  ...
  double ThinTargetMC::getMCval_pC_X(double incP, double xf,double pt, int pdgcode){
    ...
    //check:
    if(incP<12)return -1;    
    if(pdgcode!=211 && pdgcode!=-211)return -1;    
    //idx:
    int idx_part = -1;
    int idx_lowp = -1;
    int idx_hip  = -1;
    ...
    int binlow = vpC_pi[idx_part][idx_lowp]->FindBin(xf,pt);
    int binhi  = vpC_pi[idx_part][idx_hip]->FindBin(xf,pt);
    double xslow = vpC_pi[idx_part][idx_lowp]->GetBinContent(binlow);
    double xshi  = vpC_pi[idx_part][idx_hip]->GetBinContent(binhi);
