...
namespace NeutrinoFluxReweight{
  ...
  DataHistos::DataHistos(){
    ...
    DataHistos::fzpos_tgt_MC = new TFile(Form("%s/MIPP/tarpos_yield.root",dirData),"read");
    ...
    for(int ibin=0;ibin<124;ibin++){
      ...
      DataHistos::hzpostgt_pip.push_back((TH1D*)DataHistos::fzpos_tgt_MC->Get(namehist));
    }
    for(int ibin=0;ibin<119;ibin++){
      sprintf(namehist,"pim/htar_ydz_pim_bin%d",ibin);
      DataHistos::hzpostgt_pim.push_back((TH1D*)DataHistos::fzpos_tgt_MC->Get(namehist));
    }
    ...
    for(int ibin=78;ibin<124;ibin++){
      ...
      DataHistos::hzpostgt_kap.push_back((TH1D*)DataHistos::fzpos_tgt_MC->Get(namehist));
      sprintf(namehist,"kam/htar_ydz_kam_bin%d",ibin);
      DataHistos::hzpostgt_kam.push_back((TH1D*)DataHistos::fzpos_tgt_MC->Get(namehist));
    }
