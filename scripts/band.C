...
const char* corr[Ncorr]  = {"targetmesonInc","attenuation","others","total","absorption","targetpcpi","targetpck","targetncpi","targetpcnu","targetnuA", "na61pionInc"};

const char* fcorr[Ncorr] = {"ThinTargetpCPion","ThinTargetpCKaon","ThinTargetnCPion","ThinTargetpCNucleon", "ThinTargetpipCpip","ThinTargetMesonIncident","ThinTargetnucleonA","TargetAttenuation","TotalAbsorption","Other","total"};

const int corr_col[Ncorr] = {44,3,9,1,3,4,4,kOrange, kOrange,44,2};

const int corr_sty[Ncorr] = {1,1,1,1,2,1,2,1,2,2,1};

const char* cleg[Ncorr]  = {"Meson Incident","target att.","others","total HP","absorption","pC #rightarrow #piX","pC #rightarrow KX","nC #rightarrow #piX","pC #rightarrow nucleonX","nucleon-A", "#pi+ #rightarrow #pi+"};
...
const char* inputFileName = "ppfx_me000z200i_run0.root";
...
//cfe->SetLogy();    //BHUMIKA, for log scale
...
//Added by bhumika
void plot_cv_with_error_band(std::vector<TH1D*> vhistos) {
    TCanvas *c_cv = new TCanvas("c_cv", "Central Value with Error Band", 800, 600);
    TH1D *h_cv_with_error_band = get_cv_with_error_band(vhistos);
 h_cv_with_error_band->Draw("E2");

    // Customize the appearance if needed
    h_cv_with_error_band->SetTitle("Central Value with Error Band");
    h_cv_with_error_band->GetXaxis()->SetTitle("X Axis Title");
    h_cv_with_error_band->GetYaxis()->SetTitle("Y Axis Title");

    // Save or display the canvas as needed
     c_cv->Draw();

    //c_cv->SaveAs("central_value_with_error_band.png");
}
TH1D* get_cv_with_error_band(std::vector<TH1D*> vhistos){
